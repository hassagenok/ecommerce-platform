...
# make check:
После выполнения make check видно, что:
git status --short не показывает изменённых или незакоммиченных файлов
git diff --check не выводит ошибок форматирования

Это означает, что рабочее дерево находится в чистом состоянии и нет незакоммиченных изменений.
